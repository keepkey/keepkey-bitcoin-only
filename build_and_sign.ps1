...
# Check configuration
Write-Host "🔧 Checking Tauri configuration..." -ForegroundColor Yellow
...
if ($isNsisConfigured) {
    Write-Host "✅ NSIS target configured" -ForegroundColor Green
} else {
    Write-Host "❌ NSIS target not found in bundle.targets" -ForegroundColor Red
    Write-Host "Current targets: $($tauriConfig.bundle.targets -join ', ')" -ForegroundColor Gray
    exit 1
}

if ($tauriConfig.bundle.windows.nsis) {
    Write-Host "✅ NSIS settings configured" -ForegroundColor Green
    ...
    Write-Host "⚠️  NSIS settings not configured (will use defaults)" -ForegroundColor Yellow
}

# Build if not skipped
if (-not $SkipBuild) {
    Write-Host ""
    Write-Host "🔨 Building Vault v2..." -ForegroundColor Yellow
    ...
    Write-Host "⏭️  Skipping build" -ForegroundColor Blue
}
...
$bundlePath = "projects\vault-v2\src-tauri\target\release\bundle"
...
if (Test-Path $nsisPath) {
    ...
    if ($nsisFiles.Count -gt 0) {
        ...
        Write-Host "❌ No .exe files found in NSIS bundle" -ForegroundColor Red
        exit 1
    }
} else {
    Write-Host "❌ NSIS bundle not found at $nsisPath" -ForegroundColor Red
    exit 1
}
...
if (-not $SkipSign) {
    Write-Host ""
    Write-Host "✍️  Signing NSIS installer..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "💡 Make sure your Sectigo certificate is installed" -ForegroundColor Yellow
        exit 1
    }
} else {
    Write-Host "⏭️  Skipping signing" -ForegroundColor Blue
}
