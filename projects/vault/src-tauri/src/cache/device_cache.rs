...
impl DeviceCache {
    ...
    pub async fn save_features(&self, features: &crate::messages::Features, device_id: &str) -> Result<()> {
        ...
        db.execute(
            ...
            params![
                ...
                features.revision.as_ref().map(hex::encode),
                features.firmware_hash.as_ref().map(hex::encode),
                features.bootloader_hash.as_ref().map(hex::encode),
                features_json,
                now,
            ],
        )?;
        ...
        cache.features = Some(CachedFeatures {
            ...
            revision: features.revision.as_ref().map(hex::encode),
            firmware_hash: features.firmware_hash.as_ref().map(hex::encode),
            bootloader_hash: features.bootloader_hash.as_ref().map(hex::encode),
            features_json,
            last_seen: now,
        });
    ...
    pub async fn debug_load_addresses(&self, device_id: &str) -> Result<Vec<String>> {
        ...
        let rows = stmt.query_map(params![clean_device_id], |row| {
            ...
            let path_json: String = row.get(2)?;
            let address: String = row.get(3)?;
            let _pubkey: Option<String> = row.get(4)?;
...
mod tests {
    ...
    fn mock_routes_features() -> crate::messages::Features {
